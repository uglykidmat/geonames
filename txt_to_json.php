...
if(file_exists("testCountries.txt") && filesize("testCountries.txt") > 0){
    ...
    // $testCountriesArray = preg_split('/\t+/',file_get_contents("testCountries.txt",FILE_IGNORE_NEW_LINES));
    //     echo "ARRAY 1 : <pre>";
    //     var_dump ($testCountriesArray);
    //     echo "</pre>";

    $testCountriesArray = explode(PHP_EOL,file_get_contents("testCountries.txt",FILE_IGNORE_NEW_LINES));
    echo "testCountriesArray : <pre>";
    var_dump ($testCountriesArray);
    echo "</pre>";

    for ($i=0; $i < count($testCountriesArray) ; $i++) { 
        $testCountriesArray[$i] = substr($testCountriesArray[$i], 0, 2);
    }
    echo "testCountriesArray : <pre>";
    var_dump ($testCountriesArray);
    echo "</pre>";
    ...
    $countryCodeOccurrence = 0;

    foreach ($testCountriesArray as $key => $value){

    //_____________________Everytime a country Code is matched, increment occurrence value
        if (strlen($value) == 2 && in_array($value,$countryCodes)){
            $countryCodeOccurrence ++;
            
            // $countryCodes[$value] = $countryCodes[$value] + 1;
        }

        // $conents_arr[$key]  = rtrim($value, "\r");
    }
    echo "Occurrences de codeCountry : ".$countryCodeOccurrence;
    echo "<pre>";
